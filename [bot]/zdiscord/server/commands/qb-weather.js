...
module.exports = {
    ...
    run: async (client, interaction, args) => {
        if (GetResourceState("sb-weathersync") !== "started") return interaction.reply({ content: "This command requires QBCore's `sb-weathersync` to work", ephemeral: false });
        if (args.blackout) {
            // doesn't give any option for true or false or feedback to which was done -.-
            emit("sb-weathersync:server:toggleBlackout");
            ...
            emit("sb-weathersync:server:setWeather", args.weather);
