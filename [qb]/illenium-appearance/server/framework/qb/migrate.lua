...
local function MigrateQBClothing(source)
    ...
    for i=1, #allPlayerSkins, 1 do
        if not tonumber(allPlayerSkins[i].model) then
            ...
            TriggerClientEvent("illenium-appearance:client:migration:load-sb-clothing-skin", source, allPlayerSkins[i])
...
RegisterNetEvent("illenium-appearance:server:migrate-sb-clothing-skin", function(citizenid, appearance)
    local src = source
...
lib.addCommand("migrateskins", {
    ...
    elseif resourceName == "sb-clothing" then
