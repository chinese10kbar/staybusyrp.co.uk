...
RegisterServerEvent("919-admin:server:KillPlayer", function(playerId)
    ...
    if AdminPanel.HasPermission(src, "kill") then
        ...
        TriggerEvent("sb-log:server:CreateLog", "adminactions", "KICK", "red", "**STAFF MEMBER " .. GetPlayerName(src) .. "** "..Lang:t("alerts.killed").." **" .. GetPlayerName(playerId) .. "** [" .. playerId .. "]", false)
    end
end)
...
RegisterServerEvent("919-admin:server:SavePlayer", function(playerId)
    ...
    if QBCore then
        if AdminPanel.HasPermission(src, "savedata") then
            ...
            TriggerEvent("sb-log:server:CreateLog", "adminactions", "KICK", "red", "**STAFF MEMBER " .. GetPlayerName(src) .. "**" ..Lang:t("alerts.savedToDB", {value = GetPlayerName(playerId)}) .. "** [" .. playerId .. "]", false)
        end
    end
end)
...
RegisterServerEvent("919-admin:server:KickPlayer", function(playerId, reason)
    ...
    if AdminPanel.HasPermission(src, "kick") then
        TriggerEvent("sb-log:server:CreateLog", "adminactions", "KICK", "red", Lang:t("alerts.kickedPlayer", {value = GetPlayerName(src), value2 = GetPlayerName(playerId), value3 = playerId, value4 = reason}), false)
...
RegisterServerEvent("919-admin:server:Freeze", function(playerId)
    ...
    if AdminPanel.HasPermission(src, "freeze") then
        if not Frozen[playerId] then
            ...
            TriggerEvent("sb-log:server:CreateLog", "adminactions", "Frozen", "red", "**STAFF MEMBER " .. GetPlayerName(src) .. "** "..Lang:t("alerts.froze").." **" .. GetPlayerName(playerId) .. "** [" .. playerId .. "]", false)
        else
            ...
            TriggerEvent("sb-log:server:CreateLog", "adminactions", "Frozen", "red", "**STAFF MEMBER " .. GetPlayerName(src) .. "** "..Lang:t("alerts.unfroze").." **" .. GetPlayerName(playerId) .. "** [" .. playerId .. "]", false)
        end
    end
end)
            ...
            if citizenid ~= nil then
                ...
                if result[1] then
                    ...
                    if not online then
                        ...
                        TriggerEvent("sb-log:server:CreateLog", "bans", "Player Banned", "red", string.format("%s was banned by %s for %s (%s hours)", result[1].name, GetPlayerName(src), reason, time / 60 / 60), Config.TagEveryone)
                    end
                else
...
AdminPanel.OnlineBanPlayer = function(source, player, time, timeTable, reason)
    ...
    TriggerEvent("sb-log:server:CreateLog", "bans", "Player Banned", "red", string.format("%s was banned by %s for %s (%s hours)", GetPlayerName(player), GetPlayerName(src), reason, time / 60 / 60), Config.TagEveryone)
    if banTime >= 2147483647 then
            ...
            if citizenid ~= nil then
                ...
                if result[1] then
                    ...
                    if not online then
                        ...
                        TriggerEvent("sb-log:server:CreateLog", "warns", "Player Warned", "red", string.format("%s was warned by %s for %s", result[1].name, GetPlayerName(src), reason), false)
                    end
                else
...
AdminPanel.OnlineWarnPlayer = function(source, player, reason)
    ...
    TriggerEvent("sb-log:server:CreateLog", "warns", "Player Warned", "red", string.format("%s was warned by %s for %s", GetPlayerName(player), GetPlayerName(src), reason), false)
...
RegisterNetEvent("919-admin:server:CuffPlayer", function(target)
    ...
    if AdminPanel.HasPermission(src, "cuff") then
        ...
        TriggerEvent("sb-log:server:CreateLog", "adminactions", "Revive", "red", "**STAFF MEMBER " .. GetPlayerName(src) .. "** cuffed/uncuffed **" .. GetPlayerName(target) .. "** [" .. target .. "]", false)
...
RegisterServerEvent("919-admin:server:RevivePlayer", function(target)
    ...
    if AdminPanel.HasPermission(src, "revive") then
        ...
        TriggerEvent("sb-log:server:CreateLog", "adminactions", "Revive", "red", "**STAFF MEMBER " .. GetPlayerName(src) .. "** revived **" .. GetPlayerName(target) .. "** [" .. target .. "]", false)
...
RegisterServerEvent("919-admin:server:ReviveAll", function()
    ...
    if AdminPanel.HasPermission(src, "reviveall") then
        ...
        TriggerEvent("sb-log:server:CreateLog", "adminactions", "Revive", "red", "**STAFF MEMBER " .. GetPlayerName(src) .. "** revived all players.", false)
        if ESX then
...
RegisterServerEvent("919-admin:server:SetWeather", function(weatherType)
    ...
    if AdminPanel.HasPermission(src, "weather") then
        if QBCore then
            exports["sb-weathersync"]:setWeather(weatherType)
...
RegisterServerEvent("919-admin:server:SetTime", function(hour, minute)
    ...
    if AdminPanel.HasPermission(src, "time") then
        if QBCore then
            exports["sb-weathersync"]:setTime(hour, 0)
...
RegisterServerEvent("919-admin:server:FeedPlayer", function(target)
    ...
    if AdminPanel.HasPermission(src, "foodandwater") then
        ...
        TriggerEvent("sb-log:server:CreateLog", "adminactions", "Food & Water Max", "red", "**STAFF MEMBER " .. GetPlayerName(src) .. "** fed and watered **" .. GetPlayerName(target) .. "** [" .. target .. "]", false)
    end
end)
...
RegisterServerEvent("919-admin:server:RelieveStress", function(target)
    ...
    if AdminPanel.HasPermission(src, "relievestress") then
        if QBCore then
            ...
            TriggerEvent("sb-log:server:CreateLog", "adminactions", "Relieve Stress", "red", "**STAFF MEMBER " .. GetPlayerName(src) .. "** relieved stress of **" .. GetPlayerName(target) .. "** [" .. target .. "]", false)
        end
    end
end)
...
RegisterServerEvent("919-admin:server:SetMetaDataForPlayer", function(PlayerId, Meta, Data)
    ...
    if QBCore then
        ...
        TriggerClientEvent("sb-hud:client:update:needs", PlayerId, Player.PlayerData.metadata["hunger"], Player.PlayerData.metadata["thirst"])
    elseif ESX then
...
RegisterNetEvent("919-admin:server:RequestSpectate", function(player)
    ...
    if AdminPanel.HasPermission(src, "spectate") then
        ...
        TriggerEvent("sb-log:server:CreateLog", "adminactions", "Spectate", "red", "**STAFF MEMBER " .. GetPlayerName(src) .. "** started spectating **" .. GetPlayerName(player) .. "** [" .. player .. "]", false)
    end
end)
        ...
        repeat
            ...
            if Player ~= nil then
                if i == src then
                    ...
                    TriggerEvent("sb-log:server:CreateLog", "adminactions", "Spectate", "red", "**STAFF MEMBER " .. GetPlayerName(src) .. "** started spectating **" .. GetPlayerName(i) .. "** [" .. i .. "]", false)
...
RegisterNetEvent("919-admin:server:SaveCar", function(mods, vehicle, hash, plate, senderId)
    ...
    if result[1] == nil then
        ...
        if senderId then
            TriggerEvent("sb-log:server:CreateLog", "adminactions", "Admin Car", "red", "**STAFF MEMBER " .. GetPlayerName(senderId) .. "** has added a " .. vehicle.model .. " (" .. plate .. ") to the garage of " .. GetPlayerName(src), false)
            ...
            if result[1] then
                MySQL.query("DELETE FROM `" .. Config.DB.CharactersTable .. "` WHERE citizenid = ? LIMIT 1", {citizenId}, function(rowsAffected)
                    if rowsAffected then
                        ...
                        TriggerEvent("sb-log:server:CreateLog", "bans", "Player Unbanned", "red", "**STAFF MEMBER " .. GetPlayerName(src) .. "** deleted " .. result[1].name .."'s character ".. charInfo.firstname .. " " .. charInfo.lastname, false)
                    else
            ...
            if result[1] then
                MySQL.query("DELETE FROM `bans` WHERE license = ? LIMIT 1", {license}, function(rowsAffected)
                    if rowsAffected then
                        ...
                        TriggerEvent("sb-log:server:CreateLog", "bans", "Player Unbanned", "red", "**STAFF MEMBER " .. GetPlayerName(src) .. "** unbanned " .. result[1].name, false)
                    else
                        TriggerClientEvent("919-admin:client:ShowPanelAlert", src, "danger", "<strong>"..Lang:t("alerts.error").."</strong> "..Lang:t("alerts.noRowsDeleted"))
                    end
...
RegisterServerEvent("919-admin:server:ClearInventory", function(targetId)
    ...
    if AdminPanel.HasPermission(src, "clearinventory") then
        ...
        TriggerEvent("sb-log:server:CreateLog", "adminactions", "Inventory Cleared", "red", "**STAFF MEMBER " .. GetPlayerName(src) .. "** has cleared the inventory of " .. GetPlayerName(targetId), false)
...
RegisterServerEvent("919-admin:server:SetJob", function(targetId, job, grade)
    ...
    if AdminPanel.HasPermission(src, "setjob") then
        ...
        TriggerEvent("sb-log:server:CreateLog", "adminactions", "Set Job", "red", "**STAFF MEMBER " .. GetPlayerName(src) .. "** set the job of " .. GetPlayerName(targetId) .. " to " .. job .. " (" .. grade .. ")", false)
            ...
            if QBCore.Shared.Gangs[gang] ~= nil then
                if QBCore.Shared.Gangs[gang].grades[tostring(grade)] ~= nil then
                    ...
                    if targetPlayer then
                        ...
                        targetPlayer.Functions.Save()
                        TriggerEvent("sb-log:server:CreateLog", "adminactions", "Set Gang", "red", "**STAFF MEMBER " .. GetPlayerName(src) .. "** set the gang of " .. GetPlayerName(targetId) .. " to " .. gang .. " (" .. grade .. ")", false)
        ...
        if QBCore then
            if QBCore.Shared.Items[item] ~= nil then
                ...
                if targetPlayer then
                    ...
                    TriggerEvent("sb-log:server:CreateLog", "adminactions", "Set Gang", "red", "**STAFF MEMBER " .. GetPlayerName(src) .. "** gave " .. item .. " (x" .. amount .. ") to " .. GetPlayerName(targetId), false)
...
RegisterServerEvent("919-admin:server:FireJob", function(targetId)
    ...
    if AdminPanel.HasPermission(src, "firejob") then
        ...
        TriggerEvent("sb-log:server:CreateLog", "adminactions", "Set Job", "red", "**STAFF MEMBER " .. GetPlayerName(src) .. "** fired " .. GetPlayerName(targetId) .. " from their job.", false)
...
RegisterServerEvent("919-admin:server:FireGang", function(targetId)
    ...
    if AdminPanel.HasPermission(src, "firegang") then
        ...
        TriggerEvent("sb-log:server:CreateLog", "adminactions", "Set Gang", "red", "**STAFF MEMBER " .. GetPlayerName(src) .. "** removed " .. GetPlayerName(targetId) .. " from their gang.", false)
...
RegisterServerEvent("919-admin:server:FireJobByCitizenId", function(citizenId)
    ...
    if AdminPanel.HasPermission(src, "firejob") then
        ...
        if targetPlayer then
            ...
            TriggerEvent("sb-log:server:CreateLog", "adminactions", "Set Job", "red", "**STAFF MEMBER " .. GetPlayerName(src) .. "** removed " .. GetPlayerName(targetPlayer.PlayerData.source) .. " from their job.", false)
            TriggerClientEvent("919-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.firedJob"))
            ...
            if QBCore then
                ...
                MySQL.update("UPDATE `" .. Config.DB.CharactersTable .. "` SET `job` = ? WHERE `citizenid` = ?", {json.encode(PlayerData.job), citizenId}, function(rowsAffected)
                    if rowsAffected ~= 0 and rowsAffected ~= nil then
                        TriggerEvent("sb-log:server:CreateLog", "adminactions", "Set Job", "red", "**STAFF MEMBER " .. GetPlayerName(src) .. "** removed (OFFLINE) Citizen ID " .. citizenId .. " from their job.", false)
                        TriggerClientEvent("919-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.firedJob"))
                    end
                end)
            elseif ESX then
    ...
    if AdminPanel.HasPermission(src, "firegang") then
        if QBCore then
            ...
            if targetPlayer then
                ...
                targetPlayer.Functions.Save()
                TriggerEvent("sb-log:server:CreateLog", "adminactions", "Set Job Grade", "red", "**STAFF MEMBER " .. GetPlayerName(src) .. "** removed " .. GetPlayerName(targetPlayer.PlayerData.source) .. " from their gang.", false)
                TriggerClientEvent("919-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.firedGang"))
            else -- Player is offline, so we"re going to formulate the default JSON for unemployed and set it to the offline character
                PlayerData = {}
                PlayerData.gang = {}
                ...
                MySQL.update("UPDATE `" .. Config.DB.CharactersTable .. "` SET `gang` = ? WHERE `citizenid` = ?", {json.encode(PlayerData.gang), citizenId}, function(rowsAffected)
                    if rowsAffected ~= 0 and rowsAffected ~= nil then
                        TriggerEvent("sb-log:server:CreateLog", "adminactions", "Set Gang", "red", "**STAFF MEMBER " .. GetPlayerName(src) .. "** removed (OFFLINE) Citizen ID " .. citizenId .. " from their gang.", false)
                        TriggerClientEvent("919-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.firedGang"))
                    end
                end)
            end
        ...
        if QBCore then
            ...
            if targetPlayer then
                if QBCore.Shared.Gangs[targetPlayer.PlayerData.gang.name].grades[grade] ~= nil then
                    ...
                    targetPlayer.Functions.Save()
                    TriggerEvent("sb-log:server:CreateLog", "adminactions", "Set Gang Grade", "red", "**STAFF MEMBER " .. GetPlayerName(src) .. "** set gang grade of " .. GetPlayerName(targetPlayer.PlayerData.source) .. " to " .. grade .. ".", false)
                    ...
                    if gangInfo.grade ~= nil then
                        if QBCore.Shared.Gangs[gangInfo.name].grades[grade] ~= nil then
                            ...
                            MySQL.update("UPDATE `" .. Config.DB.CharactersTable .. "` SET `gang` = ? WHERE `citizenid` = ?", {json.encode(gangInfo), citizenId}, function(rowsAffected)
                                if rowsAffected ~= 0 and rowsAffected ~= nil then
                                    TriggerEvent("sb-log:server:CreateLog", "adminactions", "Set Gang", "red", "**STAFF MEMBER " .. GetPlayerName(src) .. "** set citizen id " .. citizenId .. " to gang grade " .. grade .. " (OFFLINE)", false)
        ...
        if targetPlayer then
            if QBCore then
                if QBCore.Shared.Jobs[targetPlayer.PlayerData.job.name].grades[grade] ~= nil then
                    ...
                    targetPlayer.Functions.Save()
                    TriggerEvent("sb-log:server:CreateLog", "adminactions", "Set Job", "red", "**STAFF MEMBER " .. GetPlayerName(src) .. "** set job grade of " .. GetPlayerName(targetPlayer.PlayerData.source) .. " to " .. grade .. ".", false)
                ...
                if QBCore then
                    ...
                    if QBCore.Shared.Jobs[jobInfo.name].grades[grade] ~= nil then
                        ...
                        MySQL.update("UPDATE `" .. Config.DB.CharactersTable .. "` SET `job` = ? WHERE `citizenid` = ?", {json.encode(jobInfo), citizenId}, function(rowsAffected)
                            if rowsAffected ~= 0 and rowsAffected ~= nil then
                                TriggerEvent("sb-log:server:CreateLog", "adminactions", "Set Gang", "red", "**STAFF MEMBER " .. GetPlayerName(src) .. "** set citizen id " .. citizenId .. " to job grade " .. grade .. " (OFFLINE)", false)
...
RegisterServerEvent("919-admin:server:BringPlayer", function(targetId)
    ...
    if AdminPanel.HasPermission(src, "teleport") then
        ...
        TriggerEvent("sb-log:server:CreateLog", "adminactions", "Teleport", "red", "**STAFF MEMBER " .. GetPlayerName(src) .. "** brought **" .. GetPlayerName(targetId) .. "** [" .. targetId .. "] to them", false)
    end
end)
...
RegisterServerEvent("919-admin:server:GotoPlayer", function(targetId)
    ...
    if AdminPanel.HasPermission(src, "teleport") then
        ...
        TriggerEvent("sb-log:server:CreateLog", "adminactions", "Teleport", "red", "**STAFF MEMBER " .. GetPlayerName(src) .. "** teleported to **" .. GetPlayerName(targetId) .. "** [" .. targetId .. "]", false)
    end
end)
...
RegisterServerEvent("919-admin:server:OpenSkinMenu", function(targetId)
    ...
    if AdminPanel.HasPermission(src, "skinmenu") then
        ...
        if QBCore then
            TriggerClientEvent("sb-clothing:client:openMenu", targetId)
        elseif ESX then
            TriggerClientEvent("919-admin:client:OpenSkinMenu1", targetId)
        end
        TriggerEvent("sb-log:server:CreateLog", "adminactions", "Skin Menu", "red", "**STAFF MEMBER " .. GetPlayerName(src) .. "** opened skin menu for " .. GetPlayerName(targetId), false)
    end
end)