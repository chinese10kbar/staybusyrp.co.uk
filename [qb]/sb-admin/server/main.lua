...
RegisterNetEvent("919-admin:server:AdminChatSend", function(message)
    ...
    if AdminPanel.HasPermission(src, "adminchat") then
        ...
        TriggerEvent("sb-log:server:CreateLog", "adminactions", "Admin chat send", "red", SenderName.." Has sent the following message in the adminchat: "..message.." At: "..SentTime, false)
...
RegisterNetEvent("919-admin:server:SendReport", function(subject, info, type)
    ...
    if Player then
        ...
        TriggerEvent("sb-log:server:CreateLog", "adminactions", Lang:t("alerts.reportSent"), "red", sendername..""..Lang:t("alerts.sentFollowingReport").." "..subject.." "..Lang:t("alerts.message").." "..info, false)
        for k, v in pairs(GetPlayers()) do
            v = tonumber(v)
...
RegisterServerEvent("919-admin:server:ClaimReport", function(id)
    ...
    if AdminPanel.HasPermission(src, "claimreport") then
        if AdminPanel.Reports[tonumber(id)] then
            ...
            TriggerEvent("sb-log:server:CreateLog", "adminactions", Lang:t("alerts.reportClaimed"), "red", "**STAFF MEMBER " .. GetPlayerName(src) .. "** claimed report ID "..id, false)
            for k, v in pairs(GetPlayers()) do
                v = tonumber(v)
...
RegisterServerEvent("919-admin:server:DeleteReport", function(id)
    ...
    if AdminPanel.HasPermission(src, "deletereport") then
        if AdminPanel.Reports[tonumber(id)] then
            ...
            TriggerEvent("sb-log:server:CreateLog", "adminactions", "Report Deleted", "red", "**STAFF MEMBER " .. GetPlayerName(src) .. "** deleted report ID "..id, false)
...
RegisterServerEvent("sb-log:server:CreateLog",  function(name, title, color, message, tagEveryone)
