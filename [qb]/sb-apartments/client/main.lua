...
local function OpenEntranceMenu()
    ...
    headerMenu[#headerMenu + 1] = {
        ...
        txt = '',
        params = {
            event = 'sb-menu:client:closeMenu'
        }
    }

    exports['sb-menu']:openMenu(headerMenu)
end
...
local function RegisterApartmentEntranceZone(apartmentID, apartmentData)
    ...
    zone:onPlayerInOut(function(isPointInside)
        if isPointInside and not InApartment then
            exports['sb-core']:DrawText(Lang:t('text.options'), 'left')
        else
...
local function RegisterApartmentEntranceTarget(apartmentID, apartmentData)
    ...
    exports['sb-target']:AddBoxZone(boxName, coords, boxData.length, boxData.width, {
        name = boxName,
...
local function RegisterInApartmentZone(targetKey, coords, heading, text)
    ...
    zone:onPlayerInOut(function(isPointInside)
        if isPointInside and text then
            exports['sb-core']:DrawText(text, 'left')
        else
            exports['sb-core']:HideText()
        end
...
local function RegisterInApartmentTarget(targetKey, coords, heading, options)
    ...
    local boxName = 'inApartmentTarget_' .. targetKey
    exports['sb-target']:AddBoxZone(boxName, coords, 1.5, 1.5, {
        name = boxName,
        heading = heading,
        minZ = coords.z - 1.0,
        maxZ = coords.z + 5.0,
...
local function DeleteApartmentsEntranceTargets()
    if Apartments.Locations and next(Apartments.Locations) then
        for id, apartment in pairs(Apartments.Locations) do
            if UseTarget then
                exports['sb-target']:RemoveZone('apartmentEntrance_' .. id)
            else
...
local function DeleteInApartmentTargets()
    ...
    if InApartmentTargets and next(InApartmentTargets) then
        for id, apartmentTarget in pairs(InApartmentTargets) do
            if UseTarget then
                exports['sb-target']:RemoveZone('inApartmentTarget_' .. id)
            else
    ...
    QBCore.Functions.TriggerCallback('apartments:GetApartmentOffset', function(offset)
        if offset == nil or offset == 0 then
            QBCore.Functions.TriggerCallback('apartments:GetApartmentOffsetNewOffset', function(newoffset)
                ...
                local data = exports['sb-interior']:CreateApartmentFurnished(coords)
                Wait(100)
                HouseObj = data[1]
                POIOffsets = data[2]
                InApartment = true
                ...
                Wait(100)
                TriggerServerEvent('sb-apartments:server:SetInsideMeta', house, apartmentId, true, false)
            ...
            TriggerEvent('sb-weathersync:client:DisableSync')
            Wait(100)
            TriggerServerEvent('sb-apartments:server:SetInsideMeta', house, apartmentId, true, true)
            TriggerServerEvent('InteractSound_SV:PlayOnSource', 'houses_door_close', 0.1)
            TriggerServerEvent('apartments:server:setCurrentApartment', CurrentApartment)
        end
        ...
        if new ~= nil then
            if new then
                TriggerEvent('sb-interior:client:SetNewState', true)
            else
                TriggerEvent('sb-interior:client:SetNewState', false)
            end
        else
            TriggerEvent('sb-interior:client:SetNewState', false)
        end
...
local function LeaveApartment(house)
    TriggerServerEvent('InteractSound_SV:PlayOnSource', 'houses_door_open', 0.1)
    openHouseAnim()
    TriggerServerEvent('sb-apartments:returnBucket')
    DoScreenFadeOut(500)
    while not IsScreenFadedOut() do Wait(10) end
    exports['sb-interior']:DespawnInterior(HouseObj, function()
        ...
        TriggerServerEvent('sb-apartments:server:SetInsideMeta', CurrentApartment, false)
...
function MenuOwners()
    QBCore.Functions.TriggerCallback('apartments:GetAvailableApartments', function(apartments)
        if next(apartments) == nil then
            ...
            exports['sb-menu']:openMenu(apartmentMenu)
        end
    end, ClosestHouse)
end
...
RegisterNetEvent('apartments:client:setupSpawnUI', function(cData)
    QBCore.Functions.TriggerCallback('apartments:GetOwnedApartment', function(result)
        if result then
            TriggerEvent('sb-spawn:client:setupSpawns', cData, false, nil)
            TriggerEvent('sb-spawn:client:openUI', true)
            TriggerEvent('apartments:client:SetHomeBlip', result.type)
        else
            if Apartments.Starting then
                TriggerEvent('sb-spawn:client:setupSpawns', cData, true, Apartments.Locations)
                TriggerEvent('sb-spawn:client:openUI', true)
            else
                TriggerEvent('sb-spawn:client:setupSpawns', cData, false, nil)
                TriggerEvent('sb-spawn:client:openUI', true)
...
RegisterNetEvent('sb-apartments:client:LastLocationHouse', function(apartmentType, apartmentId)
...
RegisterNetEvent('apartments:client:ChangeOutfit', function()
    ...
    TriggerEvent('sb-clothing:client:openOutfitMenu')
end)

RegisterNetEvent('apartments:client:Logout', function()
    TriggerServerEvent('sb-houses:server:LogoutLocation')
end)
