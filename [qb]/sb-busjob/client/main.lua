...
local function GetDeliveryLocation()
    ...
    PolyZone:onPlayerInOut(function(isPointInside)
        if isPointInside then
            inRange = true
            exports["sb-core"]:DrawText(Lang:t('info.busstop_text'), 'rgb(220, 20, 60)')
            CreateThread(function()
                repeat
                    ...
                    if IsControlJustPressed(0, 38) then
                        ...
                        resetNpcTask()
                        nextStop()
                        TriggerEvent('sb-busjob:client:DoBusNpc')
...
local function busGarage()
    ...
    for _, v in pairs(Config.AllowedVehicles) do
        vehicleMenu[#vehicleMenu + 1] = {
            header = v.label,
            params = {
                event = "sb-busjob:client:TakeVehicle",
                args = {
                    model = v.model
                }
            }
    ...
    exports['sb-menu']:openMenu(vehicleMenu)
end

RegisterNetEvent("sb-busjob:client:TakeVehicle", function(data)
...
RegisterNetEvent('sb-busjob:client:DoBusNpc', function()
                    ...
                    CreateThread(function()
                        repeat
                            ...
                            if IsControlJustPressed(0, 38) then
                                ...
                                TriggerServerEvent('sb-busjob:server:NpcPay')
                                exports["sb-core"]:HideText()
                                PolyZone:destroy()
                                break
                            end
            ...
            if isPointInside then
                ...
                CreateThread(function()
                    repeat
                        Wait(5)
                        if not inVeh then
                            exports["sb-core"]:DrawText(Lang:t('info.busstop_text'), 'left')
                            ...
                            exports["sb-core"]:DrawText(Lang:t('info.bus_stop_work'), 'left')
                            if IsControlJustReleased(0, 38) then
