...
QBCore.Functions.CreateCallback('sb-cityhall:server:receiveJobs', function(_, cb)
    cb(availableJobs)
end)

-- Events

RegisterNetEvent('sb-cityhall:server:requestId', function(item, hall)
    local src = source
...
RegisterNetEvent('sb-cityhall:server:sendDriverTest', function(instructors)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    for i = 1, #instructors do
        ...
        if SchoolPlayer then
            TriggerClientEvent('sb-cityhall:client:sendDriverEmail', SchoolPlayer.PlayerData.source, Player.PlayerData.charinfo)
        else
            local mailData = {
                sender = 'Township',
            ...
            exports['sb-phone']:sendNewMailToOffline(citizenid, mailData)
        end
    end
...
RegisterNetEvent('sb-cityhall:server:ApplyJob', function(job, cityhallCoords)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
...
RegisterNetEvent('sb-cityhall:server:getIDs', giveStarterItems)
