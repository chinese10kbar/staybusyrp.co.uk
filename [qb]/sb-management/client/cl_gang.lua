...
RegisterNetEvent('sb-gangmenu:client:Stash', function()
...
RegisterNetEvent('sb-gangmenu:client:Warbobe', function()
...
RegisterNetEvent('sb-gangmenu:client:OpenMenu', function()
    shownGangMenu = true
    local gangMenu = {
        {
        ...
        },
        {
            ...
            params = {
                event = 'sb-gangmenu:client:ManageGang',
            }
        },
        {
            ...
            params = {
                event = 'sb-gangmenu:client:HireMembers',
            }
        },
        {
            ...
            params = {
                event = 'sb-gangmenu:client:Stash',
            }
        },
        {
            ...
            params = {
                event = 'sb-gangmenu:client:Warbobe',
            }
        }
    }
    ...
    exports['sb-menu']:openMenu(gangMenu)
end)

RegisterNetEvent('sb-gangmenu:client:ManageGang', function()
    ...
    QBCore.Functions.TriggerCallback('sb-gangmenu:server:GetEmployees', function(cb)
        for _, v in pairs(cb) do
            GangMembersMenu[#GangMembersMenu + 1] = {
                ...
                params = {
                    event = 'sb-gangmenu:lient:ManageMember',
                    args = {
                        player = v,
                        work = PlayerGang
                    }
        ...
        GangMembersMenu[#GangMembersMenu + 1] = {
            ...
            icon = 'fa-solid fa-angle-left',
            params = {
                event = 'sb-gangmenu:client:OpenMenu',
            }
        }
        exports['sb-menu']:openMenu(GangMembersMenu)
    end, PlayerGang.name)
end)

RegisterNetEvent('sb-gangmenu:lient:ManageMember', function(data)
    local MemberMenu = {
        {
    ...
    for k, v in pairs(QBCore.Shared.Gangs[data.work.name].grades) do
        MemberMenu[#MemberMenu + 1] = {
            ...
            params = {
                isServer = true,
                event = 'sb-gangmenu:server:GradeUpdate',
    ...
    MemberMenu[#MemberMenu + 1] = {
        ...
        params = {
            isServer = true,
            event = 'sb-gangmenu:server:FireMember',
    ...
    exports['sb-menu']:openMenu(MemberMenu)
end)

RegisterNetEvent('sb-gangmenu:client:HireMembers', function()
    ...
    QBCore.Functions.TriggerCallback('sb-gangmenu:getplayers', function(players)
        for _, v in pairs(players) do
            if v and v ~= PlayerId() then
                HireMembersMenu[#HireMembersMenu + 1] = {
                    ...
                    params = {
                        isServer = true,
                        event = 'sb-gangmenu:server:HireMember',
                        args = v.sourceplayer
                    }
                }
            end
        ...
        exports['sb-menu']:openMenu(HireMembersMenu)
    end)
end)
    ...
    if Config.UseTarget then
        for gang, zones in pairs(Config.GangMenuZones) do
            for index, data in ipairs(zones) do
                exports['sb-target']:AddBoxZone(gang .. '-GangMenu' .. index, data.coords, data.length, data.width, {
                            ...
                            if #(pos - coords) < 5.0 then
                                ...
                                if #(pos - coords) <= 1.5 then
                                    ...
                                    if not shownGangMenu then
                                        exports['sb-core']:DrawText(Lang:t('drawtextgang.label'), 'left')
                                        shownGangMenu = true
                                    end

                                    if IsControlJustReleased(0, 38) then
                                        exports['sb-core']:HideText()
                                        TriggerEvent('sb-gangmenu:client:OpenMenu')
                                    end
                                end
