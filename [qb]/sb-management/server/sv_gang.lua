...
-- Get Employees
QBCore.Functions.CreateCallback('sb-gangmenu:server:GetEmployees', function(source, cb, gangname)
	local src = source
...
-- Grade Change
RegisterNetEvent('sb-gangmenu:server:GradeUpdate', function(data)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	...
	TriggerClientEvent('sb-gangmenu:client:OpenMenu', src)
end)

-- Fire Member
RegisterNetEvent('sb-gangmenu:server:FireMember', function(target)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	...
	if Employee then
		if target ~= Player.PlayerData.citizenid then
			...
			if Employee.Functions.SetGang('none', '0') then
				TriggerEvent('sb-log:server:CreateLog', 'gangmenu', 'Gang Fire', 'orange', Player.PlayerData.charinfo.firstname .. ' ' .. Player.PlayerData.charinfo.lastname .. ' successfully fired ' .. Employee.PlayerData.charinfo.firstname .. ' ' .. Employee.PlayerData.charinfo.lastname .. ' (' .. Player.PlayerData.gang.name .. ')', false)
...
-- Recruit Player
RegisterNetEvent('sb-gangmenu:server:HireMember', function(recruit)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	...
	if Target and Target.Functions.SetGang(Player.PlayerData.gang.name, 0) then
		...
		TriggerEvent('sb-log:server:CreateLog', 'gangmenu', 'Recruit', 'yellow', (Player.PlayerData.charinfo.firstname .. ' ' .. Player.PlayerData.charinfo.lastname) .. ' successfully recruited ' .. Target.PlayerData.charinfo.firstname .. ' ' .. Target.PlayerData.charinfo.lastname .. ' (' .. Player.PlayerData.gang.name .. ')', false)
	end
	TriggerClientEvent('sb-gangmenu:client:OpenMenu', src)
end)

-- Get closest player sv
QBCore.Functions.CreateCallback('sb-gangmenu:getplayers', function(source, cb)
	local src = source
	local players = {}
