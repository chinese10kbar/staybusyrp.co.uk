		...
		if v["icon"] then
			if QBCore.Shared.Items[tostring(v["icon"])] then
				if not string.find(QBCore.Shared.Items[tostring(v["icon"])].image, "//") and not string.find(v["icon"], "//") then
                    v["icon"] = "nui://sb-inventory/html/images/"..QBCore.Shared.Items[tostring(v["icon"])].image
				end
			end
		end
	end
...
RegisterNetEvent('sb-menu:client:openMenu', function(data, sort, skipFirst)
    openMenu(data, sort, skipFirst)
end)

RegisterNetEvent('sb-menu:client:closeMenu', function()
    closeMenu()
end)
...
RegisterNUICallback('closeMenu', function(_, cb)
    ...
    SetNuiFocus(false)
    cb('ok')
    TriggerEvent("sb-menu:client:menuClosed")
end)
