    ...
    if Vehicle ~= 0 then
        ...
        if not IsVehicleOnAllWheels(Vehicle) then
            VehicleMenu.items[#VehicleMenu.items+1] = {
                ...
                icon = 'car-burst',
                type = 'client',
                event = 'sb-radialmenu:flipVehicle',
                shouldClose = true
            }
        end
        ...
        if IsPedInAnyVehicle(ped) then
            ...
            for i = 1, AmountOfSeats do
                ...
                VehicleMenu.items[seatIndex].items[newIndex] = {
                    ...
                    icon = 'caret-up',
                    type = 'client',
                    event = 'sb-radialmenu:client:ChangeSeat',
                    shouldClose = false,
                }
            end
        end
...
local function controlToggle(bool)
    for i = 1, #controlsToToggle,1 do
        if bool then
            exports['sb-smallresources']:addDisableControls(controlsToToggle[i])
        else
            exports['sb-smallresources']:removeDisableControls(controlsToToggle[i])
        end
    end
end
...
local function setRadialState(bool, sendMessage, delay)
    ...
    if Config.UseWhilstWalking then
        ...
        if bool then
            TriggerEvent('sb-radialmenu:client:onRadialmenuOpen')
            SetupRadialMenu()
        else
            TriggerEvent('sb-radialmenu:client:onRadialmenuClose')
        end
        SetNuiFocus(bool, bool)
    end
...
RegisterNetEvent('sb-radialmenu:client:noPlayers', function()
    QBCore.Functions.Notify(Lang:t("error.no_people_nearby"), 'error', 2500)
end)

RegisterNetEvent('sb-radialmenu:client:openDoor', function(data)
        ...
        if closestVehicle ~= GetVehiclePedIsIn(ped) then
            ...
            if GetVehicleDoorAngleRatio(closestVehicle, door) > 0.0 then
                if not IsVehicleSeatFree(closestVehicle, -1) then
                    TriggerServerEvent('sb-radialmenu:trunk:server:Door', false, plate, door)
                else
                    SetVehicleDoorShut(closestVehicle, door, false)
                end
            else
                if not IsVehicleSeatFree(closestVehicle, -1) then
                    TriggerServerEvent('sb-radialmenu:trunk:server:Door', true, plate, door)
                else
...
RegisterNetEvent('sb-radialmenu:client:setExtra', function(data)
    local string = data.id
...
RegisterNetEvent('sb-radialmenu:trunk:client:Door', function(plate, door, open)
...
RegisterNetEvent('sb-radialmenu:client:ChangeSeat', function(data)
    local Veh = GetVehiclePedIsIn(PlayerPedId())
...
RegisterNetEvent('sb-radialmenu:flipVehicle', function()
