...
function setRadioChannel(channel, pChannelName, pFolderName)
	if GetConvarInt('voice_enableRadios', 1) ~= 1 then return end
	type_check({channel, "number"})
	if not pChannelName then pChannelName = channel end
	TriggerServerEvent('pma-voice:setPlayerRadio', channel, pChannelName)
	radioChannelName = pChannelName
	radioFolderName = pFolderName
...
RegisterCommand('+radiotalk', function()
	...
	if not radioPressed and radioEnabled then
		if radioChannel > 0 then
			...
			playerTargets(radioData, MumbleIsPlayerTalking(PlayerId()) and callData or {})
			TriggerServerEvent('pma-voice:setTalkingOnRadio', true, radioChannelName, radioFolderName)
			radioPressed = true
			playMicClicks(true)
...
function syncRadio(_radioChannel, pRadioChannelName)
	if GetConvarInt('voice_enableRadios', 1) ~= 1 then return end
	logger.info('[radio] radio set serverside update to radio %s', radioChannel)
	print('Syncing Radio Channel name to ' .. pRadioChannelName)
	radioChannel = _radioChannel
	radioChannelName = pRadioChannelName
end
